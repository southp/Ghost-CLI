...
class InstallCommand extends Command {
            ...
            await this.ui.listr([{
                ...
                task: (ctx, task) => {
                    ...
                    return yarnInstall(ctx.ui, ctx.argv.zip, {
                        noCache: ctx.argv['no-cache'],
                        onCacheHit: () => {
                            task.title = `Installing Ghost v${ctx.version} (cached)`;
                        }
                    });
                }
            }, {
    ...
    cleanInstallDirectory() {
        // Only remove the directories that ensureStructure created.
        // Deleting all of cwd (the previous behaviour) is dangerous: if this is
        // ever invoked while cwd happens to be a source tree — e.g. during tests —
        // it would wipe the entire project.
        const {installDirs} = require('../tasks/ensure-structure');
        const cwd = process.cwd();
        for (const name of installDirs) {
            fs.removeSync(path.join(cwd, name));
        }
    }
}
...
InstallCommand.options = {
    ...
    'no-cache': {
        description: 'Bypass the tarball cache; always re-download',
        type: 'boolean',
        default: false
    },
    channel: {
