...
const subTasks = {
    ...
    async cacheRead(ctx, {onCacheHit} = {}) {
        const cacheDir = getCacheDir();
        const cachedFile = path.join(cacheDir, `ghost-${ctx.version}.tgz`);
        ...
        if (shasum(data) !== ctx.shasum) {
            fs.removeSync(cachedFile);
            return;
        }

        ctx.cachedData = data; // eslint-disable-line require-atomic-updates
        if (onCacheHit) {
            onCacheHit();
        }
    },

    async download(ctx, {noCache = false} = {}) {
        let data;

        if (ctx.cachedData) {
            data = ctx.cachedData;
        } else {
            data = await download(ctx.tarball, {agent: getProxyAgent()});
            ...
            if (!noCache) {
                ...
                fs.writeFileSync(path.join(cacheDir, `ghost-${ctx.version}.tgz`), data);
            }
        }
...
module.exports = function yarnInstall(ui, archiveFile, options = {}) {
    const {noCache = false, onCacheHit} = options;
    ...
    const tasks = archiveFile ? [{
        ...
        title: 'Checking download cache',
        enabled: () => !noCache,
        task: ctx => subTasks.cacheRead(ctx, {onCacheHit})
    }, {
        title: 'Downloading',
        task: ctx => subTasks.download(ctx, {noCache})
    }];
